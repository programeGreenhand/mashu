/* MaShu Coding Agent 桌面端 UI
 *
 * 此文件是 app/static/app.js 的副本，差异：
 *   - API 基础地址走 window.mashu.apiBase（由 preload 注入）
 *   - 由 Electron 自定义协议 mashu:// 直接加载，不再经 FastAPI
 *
 * 同步修改时请保持 app/static/app.js 与本文件一致（或反过来）。
 */
(() => {
    'use strict';

    const API = (window.mashu && window.mashu.apiBase) || '';
    ...
    async function sendMessage(text) {
        ...
        try {
            const resp = await fetch(`${API}/api/sessions/${state.sessionId}/chat`, {
                method: 'POST',
